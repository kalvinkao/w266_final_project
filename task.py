...
class Evaluator(object):
  ...
  def evaluate(self, num_eval_batches=None):
    ...
    with tf.Graph().as_default() as graph:
      self.tensors, self.to_pass = self.model.build_eval_graph(self.eval_data_paths, 
  ...
  def run_training(self):
    ...
    with tf.Graph().as_default() as graph:
      with tf.device(device_fn):
        ...
        self.tensors, self.to_pass = self.model.build_train_graph(self.args.train_data_paths, 
                                                                  self.args.batch_size)
    ...
    #to_run_1 = [self.tensors.global_step, self.tensors.train, self.to_pass.loss, self.to_pass.initial_h]
    #to_run = [self.tensors.global_step, self.tensors.train, self.to_pass.loss, self.to_pass.final_h]
    to_run = [self.tensors.global_step, self.tensors.train, self.to_pass.loss, self.to_pass.final_h]
    ...
    while should_retry:
      try:
        ...
        with self.sv.managed_session(target, config=config) as session:
          ...
          #ADD A WAY TO SET V AFTER LOADING DATA
          #load data here        
          #five_star_reviews = get_review_series(self.model.data_paths)
          five_star_reviews = get_review_series(self.args.train_data_paths)
          #train_x_review_list, train_y_review_list = make_training_data(five_star_reviews, training_samples=20000)
          train_review_list = make_train_data(five_star_reviews, training_samples=20000)
          #words_to_ids, ids_to_words = make_vocabulary(train_x_review_list, train_y_review_list)
          words_to_ids, ids_to_words = make_vocabulary(train_review_list, train_review_list)
          #revert back to make_train_test_data
          #train_x = convert_to_ids(words_to_ids, train_x_review_list)
          #train_y = convert_to_ids(words_to_ids, train_y_review_list)
          train_ids = convert_to_ids(words_to_ids, train_review_list)
          bi = rnnlm_batch_generator(train_ids, batch_size=256, max_time=300)
          
          #get w and y from bi here
          #wrap the below in a for loop with bi?
          #while not self.sv.should_stop() and self.global_step < max_steps:
          learning_rate = 0.002
          use_dropout = True
          for i, (w, y) in enumerate(bi):
              if not self.sv.should_stop() and self.global_step < max_steps:
                  try:
                    # Run one step of the model.
                    #if self.global_step == 0:
                    if self.local_step == 0:
                        h = session.run([self.model.initial_h_, {self.model.input_w_: w}])
                    
                    feed_dict = {self.model.input_w_:w, 
                                 self.model.target_y_:y, 
                                 self.model.learning_rate_: learning_rate, 
                                 self.model.use_dropout_: use_dropout, 
                                 self.model.initial_h_:h}
                    #self.global_step, _, cost, h = session.run(to_run)
                    #self.global_step = session.run(to_run)[0]
                    self.global_step, _, cost, h = session.run(to_run, feed_dict=feed_dict)
                    self.local_step += 1
          ...
          if self.is_master:
            ...
            self.model.export(tf.train.latest_checkpoint(self.train_path), self.model_path, words_to_ids, ids_to_words)
...
def get_review_series(review_path):
...
def make_train_data(five_star_review_series, training_samples=20000):
    #add randomization
...
def make_training_data(five_star_review_series, training_samples=20000):
    #add randomization
    review_list = preprocess_review_series(five_star_review_series)
    train_x_review_list = [item for sublist in review_list[:training_samples] for item in sublist[:-1]]
    print(len(train_x_review_list))
    train_y_review_list = [item for sublist in review_list[:training_samples] for item in sublist[1:]]
    return train_x_review_list, train_y_review_list
